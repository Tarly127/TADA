package utils.consensus.synchConsensusUtils;
...
public final class FCAInstance extends ConsensusState implements ConsensusInstance<Double>
{
    ...
    private final AtomicBoolean consensusPossible;
        ...
        return broadcast(vAtCurrentRound, round, MessageType.FCA_APPROXIMATION, this.reqID)
                .thenApply(nothing ->
                {
                    ...
                    if(!timedOut)
                    {
                        ...
                        {
                            if(Math.abs(V.length - A.length) >= this.t)
                            {
                                // too many faults were detected!
