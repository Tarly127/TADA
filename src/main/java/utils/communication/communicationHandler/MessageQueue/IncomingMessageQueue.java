...
public class IncomingMessageQueue implements MessageQueue
{
    ...
    public Subscription getSubscription(final Collection<Byte> acceptableTypes, final int regId)
    {
    ...
    public Subscription getSubscription(final Collection<Byte> acceptableTypes,
    ...
    public boolean addTypesToRegistration(Collection<Byte> newTypes, Subscription subscription)
    {
        ...
        if(this.registeredAcceptableTypes.containsKey(subscription))
        {
            // check if is associated with instanceID
            if(this.registeredInstanceIDs.containsKey(subscription))
            {
                var prevRegisteredTypes = this.registeredAcceptableTypes.get(subscription);

                for(var type : newTypes)
                {
                    if(!prevRegisteredTypes.contains(type))
                    {
                        prevRegisteredTypes.add(type);
                        added = true;
                    }
                }
            }
            else
            {
                for(var type : newTypes)
                {
                    if (noRepeatedTypesRegistered(Collections.singleton(type)))
                    {
                        this.registeredAcceptableTypes.get(subscription).add(type);
                        added = true;
                    }
                }
            }
